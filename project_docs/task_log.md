...
## 2026-04-30 GitHub Pages Web 发布准备

- 下载并安装 RenPyWeb 支持包到临时 SDK `/tmp/renpy-8.3.7-sdk/web`。
- 使用命令行成功生成 Web/HTML5 构建目录 `dragon_pact_renpy_web_build/`。
- 修改 `game/options.rpy` 构建分类规则，排除开发文档、原始素材备份、禁用视频、参考语音与候选语音，避免 Web 包体超过 GitHub 单文件限制。
